...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
    ...
    protected void enforceRBTreePropertiesAfterInsert(Node<T> newNode) {
       Node<T> parentNode = newNode.context[0]; // ref to parent
       if (parentNode == null) { // if parent null, new node is root and black
       ...
       Node<T> grandparent = parentNode.context[0];
       // call helper method
       Node<T> uncle = getUncleNode(parentNode);
       // if uncle red, recolor
       if (uncle != null && uncle.blackHeight == 0) {
        parentNode.blackHeight = 1;
        grandparent.blackHeight = 0;
        uncle.blackHeight = 1;
        // call for grandparent again
        enforceRBTreePropertiesAfterInsert(grandparent);
       }
       else if (parentNode == grandparent.context[1]) {
        if (newNode.isRightChild()) {
            // rotate, and then recolor
            rotate(newNode, parentNode);
            parentNode = newNode;
        }
        rotate(parentNode, grandparent);

        parentNode.blackHeight = 1;
        grandparent.blackHeight = 0;
       }
       else {
        if (newNode == parentNode.context[1]) {
            rotate(newNode, parentNode);
    ...
    private Node<T> getUncleNode(Node<T> parentNode) {
        Node<T> grandparent = parentNode.context[0];
        if (grandparent.context[1] == parentNode) {
    ...
    private void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
        ...
        if (child.context[0] != parent) {
            throw new IllegalArgumentException("Cannot rotate operation on provided nodes: no parent-child rotation");
        }
        if (child.isRightChild()) { // if the child is a right child, then we do a left rotation
    ...
    @Test
    /**
     * This method tests to ensure that when a new node is inserted into an empty RedBlackTree, the node is correctly colored
     */
    public void testInsertRoot() {
        _instance.insert(1);
        assertEquals(1, _instance.root.blackHeight);
    }
    @Test
    /**
     * This method tests to ensure RedBlack tree properties are followed when 5 insertions occur.
     */
    public void testInsert5() {
        ...
        _instance.insert(5);
        assertEquals(1, _instance.root.blackHeight);
        assertEquals(1, _instance.root.context[1].blackHeight);
        assertEquals(1, _instance.root.context[2].blackHeight);
        assertEquals(0, _instance.root.context[2].context[1].blackHeight);
        assertEquals(0, _instance.root.context[2].context[2].blackHeight);
    }

    @Test
    /**
     * Tests recoloring when both new node and parent are red, and uncle is red.
     */
    public void testRecolor() {
        ...
        _instance.insert(5);
        _instance.insert(10);
        assertEquals(0, _instance.root.context[2].blackHeight);
        assertEquals(1, _instance.findNodeWithData(3).blackHeight);
        assertEquals(1, _instance.findNodeWithData(5).blackHeight);
        assertEquals(0, _instance.findNodeWithData(10).blackHeight);
    }
